...
function mapRowToLead(row) {
      ...
      const isCompanyName = v => v.length >= 3 && !isPureNum(v) && !isJunkHeader(v) && !isEmail(v) && !isPhone(v) && hasLetters(v) && !isJobTitle(v);
      ...
      if (allGeneric) {
        ...
        for (const v of vals) {
          ...
          else if (!name && isCompanyName(v)) { name = v; }
      ...
      if (region && !hasLetters(region)) region = '';
...
leads.forEach((l, i) => {
  if (l.name.toLowerCase().includes('vertriebsdirektor') || l.ceos.includes('Vertriebsdirektor')) {
    console.log(`\nLead [Vertriebsdirektor] ${i}: Name="${l.name}" Region="${l.region}" CEO="${l.ceos}"`);
  }
  if (l.name.includes('Unbekannt')) {
    console.log(`\nLead [Unbekannt] ${i}: Name="${l.name}" Region="${l.region}"`);
  }
  if (l.region && l.region.includes('1828')) {
    console.log(`\nLead [1828] ${i}: Name="${l.name}" Region="${l.region}" Phone="${l.phone}"`);
  }
});

console.log('\n=== ALL MAPPED LEADS (first 5) ===');
leads.slice(0, 5).forEach((l, i) => {
  console.log(`Lead ${i}: Name="${l.name}" Region="${l.region}" CEO="${l.ceos}" phone="${l.phone}"`);
});
